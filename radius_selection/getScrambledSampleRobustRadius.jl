...
# Determine a covariate-independent Sample Robust radius (Algorithm 2) using a ER-SAA-based DRO problem
function getSampleRobustRadius(returns_data::Array{Float64,2},covariates_data::Array{Float64,2},TVRadii::Array{Float64,1},regressionMethod::String,coeff_true::Array{Float64,2})

	numSamples::Int64 = size(returns_data,1)	
	numRadii::Int64 = size(TVRadii,1)
	numFolds::Int64 = 5
	...
	for f = 1:numFolds
		...
		for cov = 1:numCovariatePoints
			...
			for r = 1:numRadii
				z_soln_tv, ~ = solveSampleRobustModel(returns_scen_fold,TVRadii[r]) 
				solnCostsRadii[r] += (estimateCostOfSoln(z_soln_tv,returns_perm[index_folds[f,1]:index_folds[f,2],:]))/(numFolds*numCovariatePoints)
			end			
		end
	end

	return TVRadii[indmin(solnCostsRadii)], solnCostsRadii
end
