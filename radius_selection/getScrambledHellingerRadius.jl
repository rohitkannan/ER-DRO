...
# Determine a covariate-independent Hellinger radius (Algorithm 2) using a ER-SAA-based DRO problem
function getHellingerRadius(returns_data::Array{Float64,2},covariates_data::Array{Float64,2},hellingerRadii::Array{Float64,1},regressionMethod::String,coeff_true::Array{Float64,2})

	numSamples::Int64 = size(returns_data,1)	
	numRadii::Int64 = size(hellingerRadii,1)
	numFolds::Int64 = 5
	...
	# solve the ER-SAA-based Hellinger-DRO problem with the data for each fold omitted
	...
	for f = 1:numFolds
		...
		for cov = 1:numCovariatePoints
			...
			for r = 1:numRadii
				z_soln_hellinger, ~ = solveHellingerModel(returns_scen_fold,hellingerRadii[r]) 
				solnCostsRadii[r] += (estimateCostOfSoln(z_soln_hellinger,returns_perm[index_folds[f,1]:index_folds[f,2],:]))/(numFolds*numCovariatePoints)
			end			
		end
	end

	return hellingerRadii[indmin(solnCostsRadii)], solnCostsRadii
end
