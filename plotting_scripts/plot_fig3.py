# Python script for plotting ER-SAA + Lasso vs Wasserstein ER-DRO + Lasso using Algorithms 1, 2, and 3
import numpy as np
	...
	for samp_size in range(num_sample_sizes):
		for deg in range(num_degrees):
			for data_rep in range(num_data_replicates):
				with open("case" + str(4+cov_dim) + "_ersaa/mod_1/deg_" + str(deg+1) + "/rep_" + str(data_rep+1) + "/lasso/samp_" + str(ERSAA_samp_num[cov_dim][samp_size]+1) + "/ersaa_obj.txt") as ERSAA_file:
...
# Next, load the Wasserstein ER-DRO + Lasso data when Algorithm 1 is used to specify the radius
...
for cov_dim in range(num_covariate_dimensions):
	for samp_size in range(num_sample_sizes):
		for deg in range(num_degrees):
			for data_rep in range(num_data_replicates):
				with open("case" + str(4+cov_dim) + "_wass_naive/mod_1/deg_" + str(deg+1) + "/rep_" + str(data_rep+1) + "/lasso/samp_" + str(wass_samp_num[cov_dim][samp_size]+1) + "/wass_obj.txt") as wass_file:
...
# Next, load the Wasserstein ER-DRO + Lasso data when Algorithm 2 is used to specify the radius
...
for cov_dim in range(num_covariate_dimensions):
	for samp_size in range(num_sample_sizes):
		for deg in range(num_degrees):
			for data_rep in range(num_data_replicates):
				with open("case" + str(4+cov_dim) + "_wass_scrambled/mod_1/deg_" + str(deg+1) + "/rep_" + str(data_rep+1) + "/lasso/samp_" + str(wass_samp_num[cov_dim][samp_size]+1) + "/wass_obj.txt") as wass_file:
					wass_list = wass_file.read().splitlines() 
					for cov_rep in range(num_covariate_replicates):
...
# Next, load the Wasserstein ER-DRO + Lasso data when Algorithm 3 is used to specify the radius
...
for cov_dim in range(num_covariate_dimensions):
	for samp_size in range(num_sample_sizes):
		for deg in range(num_degrees):
			missing_data = []
			for data_rep in range(num_data_replicates):
				with open("case" + str(4+cov_dim) + "_wass_doubleest/mod_1/deg_" + str(deg+1) + "/rep_" + str(data_rep+1) + "/lasso/samp_" + str(wass_samp_num[cov_dim][samp_size]+1) + "/wass_obj.txt") as wass_file:
					wass_list = wass_file.read().splitlines() 
...
# Plot ER-SAA + OLS vs Wasserstein ER-DRO + OLS using Algorithms 1, 2, and 3
box_colors = ['limegreen', 'black', 'blue', 'red']
plot_y_max = np.array([[70, 70, 70], [45, 40, 50], [10, 10, 12]], dtype=np.int32)
for deg in range(num_degrees):
	for cov in range(num_covariate_dimensions):
		...
		for meth in range(4):
			...
			plt.savefig("figures/fig3_cov" + str(cov+1) + "_deg" + str(deg+1) + ".eps", format='eps')
