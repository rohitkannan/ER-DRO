# Python script for plotting Wasserstein ER-DRO + OLS using Algorithms 2 and 3 versus optimal covariate independent and dependent specifications of the radii
import numpy as np
		...
		for deg in range(num_degrees):
			for data_rep in range(num_data_replicates):
				for risk_num in range(num_risk_levels):
					with open("case" + str(1+cov_dim) + "_wass_tailored/mod_1/deg_" + str(deg+1) + "/rep_" + str(data_rep+1) + "/ols/samp_" + str(wass_samp_num[cov_dim][samp_size]+1) + "/risk_" + str(risk_num+1) + "/wass_obj.txt") as wass_file:
						wass_list = wass_file.read().splitlines() 
						for cov_rep in range(num_covariate_replicates):
				...
				opt_risk_num_indep = np.argmin(wass_ERDRO_tailored_indep_crit[cov_dim,deg,data_rep,samp_size,:])
				wass_ERDRO_tailored_ucb_indep[cov_dim,deg,:,data_rep,samp_size] = wass_ERDRO_tailored_ucb[cov_dim,deg,:,data_rep,samp_size,opt_risk_num_indep]
				for cov_rep in range(num_covariate_replicates):
					opt_risk_num_dep = np.argmin(wass_ERDRO_tailored_ucb[cov_dim,deg,cov_rep,data_rep,samp_size,:])
					wass_ERDRO_tailored_ucb_dep[cov_dim,deg,cov_rep,data_rep,samp_size] = wass_ERDRO_tailored_ucb[cov_dim,deg,cov_rep,data_rep,samp_size,opt_risk_num_dep]


# Plot Wasserstein ER-DRO + OLS using Algorithms 2 and 3 versus optimal covariate independent and dependent specifications of the radii
box_colors = ['limegreen', 'black', 'blue', 'red']
plot_y_max = [20.0, 20.0, 20.0]
for deg in range(num_degrees):
		...
		for meth in range(4):
			...
			for samp_size in range(num_sample_sizes):
				if meth == 0:
					...
					data[:,samp_size] = wass_ERDRO_tailored_ucb_indep[cov,deg,:,:,samp_size].flatten()
					if (samp_size == 0 and cov == 0):
					...
					data[:,samp_size] = wass_ERDRO_tailored_ucb_dep[cov,deg,:,:,samp_size].flatten()
			...
			for samp_size in range(num_sample_sizes):
				...
				box_labels_overall[5*samp_size+1] = r'I$\!^*$'
				...
				box_labels_overall[5*samp_size+4] = r'D$\!^*$'
			...
			plt.savefig("figures/fig6_cov" + str(cov+1) + "_deg" + str(deg+1) + ".eps", format='eps')
