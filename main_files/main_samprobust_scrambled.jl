# main file for solving the Sample Robust Optimization problem using Algorithm 2 for specifying the radius

using Distributions, StatsFuns, StatsBase, JuMP, Gurobi, GLMNet, Convex, SCS
...
#*========= MODEL INFORMATION ==========
const caseNum = 1
const paramsFile = "params_case" * string(caseNum) * "_samprobust_scrambled.jl"
#*======================================
...
include("solveSampleRobustModel.jl")
include("estimateSolnCost.jl")
include("evaluateSoln.jl")
include("getScrambledSampleRobustRadius.jl")
#*======================================
...
const baseDirName = "case" * string(caseNum) * "_samprobust_scrambled/" * "mod_" * string(modRepNum) * "/" * "deg_" * string(degNum) * "/"
...
const tvObjFile = "samprobust_obj.txt"
const tvDDObjFile = "samprobust_ddobj.txt"
const tvTimeFile = "samprobust_time.txt"
const radiusTimeFile = "radius_time.txt"
const tvRadiusFile = "samprobust_radius.txt"
...
if(storeResults)
	...
	open(details_file, "w") do f
		...
		write(f,"Ambiguity set radius: $sampRobustRadius \n")
...
#*========= GET SAMPLE ROBUST RADIUS ==============

tic()

optTVRadius, radiusQuality = getSampleRobustRadius(returns_data,covariate_data,sampRobustRadius,regressionMethod,coeff_true)
...
if(storeResults) 
	...
	radius_file = subDirName * tvRadiusFile		
...
#*========= SOLVE SAMPLE ROBUST OPTIMIZATION MODEL ====================
	...
	z_soln_tv, objDDTV = solveSampleRobustModel(returns_scen_ersaa,optTVRadius)

	TVObjEstimates = estimateSolnQuality(z_soln_tv,covariate_obs,degree[degNum],numMCScenarios,numMCReplicates,coeff_true)
...
#*========= STORE RESULTS ====================
	if(storeResults)
		...
		obj_est_file = subDirName * tvObjFile
		open(obj_est_file, "a") do f
			for i = 1:length(TVObjEstimates)
				write(f,"$(TVObjEstimates[i]) \n")
			end
		end	
	
		ddobj_est_file = subDirName * tvDDObjFile
		open(ddobj_est_file, "a") do f
			write(f,"$objDDTV \n")
		end	
	
		time_obj_file = subDirName * tvTimeFile
